...
namespace CapaPresentacion.Producto
{
    public partial class frm_Editar_Precios_CompraVenta : Form
    {
        ...
        private void SeleccionarProducto()
        {
            ...
            {
                filtro.Show();
                ver.lbl_msm.Text = "Selecciona el Item para Editar";
                ver.ShowDialog(this);
                filtro.Hide();
            }
            else
            {
                ...
                string nombreprod = lis.SubItems[1].Text;
                double stockProd = Convert.ToDouble(lis.SubItems[2].Text);
                double precompra1 = Convert.ToDouble(lis.SubItems[3].Text);

                frm_Cambiar_PreVenta cant = new frm_Cambiar_PreVenta();
                cant.idProduto = idprod;

                filtro.Show();

                cant.lbl_producto_precio.Text = nombreprod;
                cant.ShowDialog(this);
                filtro.Hide();

                if (cant.Tag != null && cant.Tag.ToString() == "A")
                {
                    double precompra = Convert.ToDouble(cant.txt_precio_compra.Text);
                    double preventa = Convert.ToDouble(cant.txt_precio_venta.Text);

                
                    obj.CN_Actualizar_PrecioCompra_Producto(idprod, precompra, preventa, 0, 0);
